...
class DiffusionModel(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # нормальное распределение надо, было rand_like
        eps = torch.randn_like(x, device=x.device)

        # вот тут косячок, второе слагаемое другое должно быть
        x_t = (
    ...
    def sample(self, num_samples: int, size, device) -> torch.Tensor:
        ...
        # опять приколы с девайсом
        x_i = torch.randn(num_samples, *size, device=device)
        noise = x_i.clone()

        for i in trange(self.num_timesteps, 0, -1, desc="Generating samples", leave=False):
